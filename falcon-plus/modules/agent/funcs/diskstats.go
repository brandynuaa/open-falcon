...
func DiskIOMetrics() (L []*model.MetricValue) {
	...
	for _, ds := range dsList {
		...
		component := ds.Device
		metricGroup := "DiskIOMonitor"
		unit := "a"

		L = append(L, CounterValueAIOPS("disk.io.reads", ds.ReadRequests, component, metricGroup, unit, device))
		...
		L = append(L, CounterValueAIOPS("disk.io.writes", ds.WriteRequests, component, metricGroup, unit, device))
...
func IOStatsMetrics() (L []*model.MetricValue) {
	...
	for device := range diskStatsMap {
		...
		unitms := "ms"
		unitPer := "%"
		L = append(L, GaugeValueAIOPS("disk.io.read_bytes", float64(delta_rsec)*512.0, component, metricGroup, unit, tags))
		L = append(L, GaugeValueAIOPS("disk.io.write_bytes", float64(delta_wsec)*512.0, component, metricGroup, unit, tags))
		...
		// L = append(L, GaugeValue("disk.io.avgrq_sz", avgrq_sz, tags))
		// L = append(L, GaugeValue("disk.io.avgqu-sz", float64(IODelta(device, IOMsecWeightedTotal))/1000.0, tags))
		L = append(L, GaugeValueAIOPS("disk.io.await", await, component, metricGroup, unitms, tags))
		// L = append(L, GaugeValue("disk.io.svctm", svctm, tags))
		tmp := float64(use) * 100.0 / float64(duration)
		if tmp > 100.0 {
			tmp = 100.0
		}
		L = append(L, GaugeValueAIOPS("disk.io.util", tmp, component, metricGroup, unitPer, tags))
	}
